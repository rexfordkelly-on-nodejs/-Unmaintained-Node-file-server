...
function Directory() {
  ...
  this.toMiddleware = function toMiddleware(options) {
    ...
    function lactateMiddleware() {
      this.serve.apply(this, arguments);
    };
    
    return lactateMiddleware.bind(this);
  };
  ...
  this.serveIndex = function serveIndex(fp, req, res, error) {
    ...
    fs.stat(fp, function statCallback(err, stat) {
      if (err) {
        error(500);
      } else if (stat.isFile()) {
        self.serve(fp, req, res);
      } else {
    ...
    function readCallback(err, files) {
      ...
      if (lastSlash) files.unshift('Parent directory'.link(parent));
