...
module.exports.dir = function(directory, options) {
  ...
  if (bundle) {
    ...
    switch(typeof(bundle)) {
      case 'boolean':
        lactate.bundleScripts();
        lactate.bundleStyles();
        break;
      case 'string':
...
function Directory() {
  ...
  this.bundle = function (type, name, cb) {
    name = name || ['common', type].join('.');
    var location = appendRoot(name);

    var files;
    if (type instanceof Array) {
      files = type.map(appendRoot);
    } else {
      files = filterFiles(type);
    };
    ...
    abridge.minify(files, location, function() {
      if (cb) cb.apply(this, arguments);
      if (rebundle) files.forEach(watch);
    });
  };
