...
Lactate.prototype.serve = function(fp, req, res, status) {
    ...
    if (opts.expires && ims === mtime) {
        ...
        if (!status || status === 200) {
            self._200('Cached', res, cachedFile.headers, cachedFile.data);
        }else {
...
Lactate.prototype.send = function(filePath, headers, res, _404) {
    ...
    rs.on('open', function(fd) {
        if (_404) {
            ...
            self._200('Read and served file', res, headers)
        }
        rs.pipe(gz).pipe(mux);
    })
...
Lactate.prototype._200 = function(msg, res, headers, data) {
    var status = 200
...
Lactate.prototype._304 = function(res, filePath) {
    ...
    res.end()
    log[status]('Not modified', filePath);
};
