...
Lactate.prototype.complete = function(filePath, mtime, res, status, sync) {
...
Lactate.prototype.send = function(filePath, headers, res, _404) {
    ...
    if (isText) {
        var stream = (this.opts.minify && isJs)
        ? rs.pipe(utils.createMinify())
        : rs;

        if (this.opts.gzip) {
            headers['Content-Encoding'] = 'gzip';
            stream.pipe(zlib.createGzip()).pipe(mux);
        };
    }else {
        rs.pipe(mux);
    };
...
module.exports.dir = function(directory, options) {
    ...
    lactate.combineScripts = function(name) {
        var root = this.get('root');
        var location = path.join(root, name);
        var comp = utils.combineScripts(root);
        return fs.writeFileSync(location, comp);
    };

    var combine = options['combineScripts'];
    if (combine && typeof(combine) === 'string') {
        lactate.combineScripts(combine);
    };
