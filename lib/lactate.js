...
Lactate.prototype.serve = function(fp, req, res, status) {
  ...
  var _404 = this._404.bind(this, res);
  ...
  if (typeof(fp) === 'string') {
     ...
     */
    if (dirname !== public) {
      if (!opts.subdirs) return _404(url);
      var subdir = dirname.substring(0, pubLen);
      if (subdir !== public) return _404(url);
    };
  ...
  var ims = !!req ? req.headers[_ims] : null;
...
Lactate.prototype.send = function(filePath, headers, res, _404) {
   ...
   */
  var dataStreamed = function(data) {
    if (this.opts.cache) {
  ...
  var mux = new Suckle(dataStreamed);
  if (!!res) mux.pipe(res);
