  ...
  this.serveIndex = function(fp, req, res) {
    ...
    function readCB(err, files) {
      ...
      .forEach(function(file) {
        try {
          var _filePath = appendRoot(file);
          var _file = fs.statSync(_filePath);
      ...
      .concat(_files.sort())
      .map(function(file) {
        return el('a', file, 'href="' + path.join(_fp, file) + '"');
      })
      .map(function(file) {
        return el('li', file);
      });
      ...
      if (_fp !== '/') {
        files.unshift(el('a', 'Parent directory', 'href="' + parent + '"'));
      };

      var content = el('h1', 'Index of ' + _fp)
      + el('ul', files.join(''));
