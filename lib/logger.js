...
module.exports.createDebugger = function() {
  var listen = function(status) {
    ...
    this.on(status, function(ev) {
      self.log(status, ev.msg, ev.url, ev.path);
    });
  };

  var codes = Object.keys(this.STATUS_CODES);
  codes.forEach(listen.bind(this));
};

function Logger(options) {};

Logger.prototype.log = function(status, msg, path) {
  var msg = this.msg(msg);
  var _status = this.status(status);
  var str = [_status, msg, path].join(' ');
  console.log(str);
};
